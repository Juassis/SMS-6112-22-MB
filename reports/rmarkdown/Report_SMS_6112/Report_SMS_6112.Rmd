...
cd /proj/snic2022-22-672/private/SMS_6112_RNASeq_Bat/ 
...
samples_info <- read.table(paste0(path, "data/sample_data.tsv"), header=T, check.names=T, sep="\t")
...
samples_info$organ_bat <- paste0(samples_info$organ, '_', samples_info$bat, '_')
...
exp <- readRDS("/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/salmon.merged.gene_counts_length_scaled.rds")
...
a <- plotPCA(vsd, intgroup = c('sample_id', 'bat', 'organ' ,'library','organ_bat'), returnData = TRUE)


# By organ
png(paste0(path, 'results/salmon/results/02-DE/PCA_bat_lremoved.png'), width = 4000, height = 3000, res = 300)
b <- ggplot(a, aes(x = PC1, y = PC2, color = organ, shape = bat)) + 
  geom_point(size = 4) +
  theme_pubr(border = TRUE) +
  geom_text_repel(aes(label = library), nudge_x = 0.06, size = 3.0, segment.alpha = 0.5)
b
dev.off()

# By library
png(paste0(path, 'results/salmon/results/02-DE/PCA_library_removed.png'), width = 4000, height = 3000, res = 300)
c <- ggplot(a, aes(x = PC1, y = PC2, color = library, shape = organ)) + 
  geom_point(size = 4) +
  theme_pubr(border = TRUE) +
  geom_text_repel(aes(label = bat), nudge_x = 0.06, size = 3.0, segment.alpha = 0.5) 
c
dev.off()
#
...
# par( mfrow = c( 1, 2 ) )
# plot( log2( 1+counts(dds, normalized=FALSE)[, 1:2] ), col="#00000020", pch=20, cex=0.3 )
...
png(paste0(path, 'results/salmon/results/02-DE/HeatMap_rlog.png'), width = 4000, height = 3000, res = 300)
...
df$bat = factor(df$bat, levels = c("Bat_1 ", "Bat_2 ","Bat")) #D46C4E
metaR_bat <- c("#C0C0C0","#BC8F8F","black") #D46C4E #43978D F5DEB3
names(metaR_bat) <- levels(df$bat)

df$organ = factor(df$organ, levels = c("organ_Skeletal muscle", "organ_Intestine","organ_Kidney")) #D46C4E
metaR_organ <- c("#77A515", "#264D59", "#43978D") #D46C4E #43978D
...
png(paste0(path, 'results/salmon/results/02-DE/HeatMap_topvar100.png'), width = 4000, height = 8000, res = 300)
...
png(paste0(path, 'results/salmon/results/02-DE/HeatMap_topvar100_clus.png'), width = 5000, height = 8000, res = 300)
...
#
# topVarGenes <- order(-rowVars(assay(rld)))[0:100]
# mat <- assay(rld)[ topVarGenes, ]
...
# hist(zscores[ ,5 ]) # where idx is the number of the sample
# gene.idx <- which(zscores[,5] > 1) # where x is a large value
...
rownames(sampleDistMatrix2) <- paste( rld2$sample_id, rld2$organ, rld2$bat , sep="-" )

cor <- cor((assay(vsd)))
rownames(cor) <- rownames(sampleDistMatrix2)
#rownames(sampleDists2) <- vsd$sample_name


#blue_c <- generate_palette("blue", modification = "go_darker", n_colors = 255, view_palette = FALSE, view_labels = FALSE)
#colors <- colorRampPalette(rev(brewer.pal(9, 'Blues')))(255)

colors <- colorRampPalette(brewer.pal(9, 'Blues'))(255)
png(paste0(path, 'results/salmon/results/02-DE/samples_correlation.png'), width =3000, height = 2500, res = 300)
correlation <- pheatmap(cor, clustering_distance_rows = 'correlation', clustering_distance_cols = 'correlation', color = colors)
correlation
dev.off()
```
...
ggplot(counts_differexp, aes(x = bat, y = log2(expression), fill = factor(organ))) + geom_violin()
```

```{r cut-off2, eval = F, echo = F}

ddsColl <- collapseReplicates(dds, dds$sample_name, dds$library)

colData(ddsColl)
colnames(ddsColl)

matchFirstLevel <- dds$sample_name == levels(dds$sample_name)[1]
...
png(paste0(path, 'results/salmon/results/02-DE/distribution.png'), width = 3000, height = 2500, res = 300)
ggplot(data = counts_norm, aes(x = reorder(sample_name, bat, na.rm = T), y = log2(counts+1))) +  geom_boxplot(aes(fill = factor(counts_norm$organ))) + coord_flip() + xlab('sample_name')
dev.off()
```

# Results  
## Filtering
We filter out low-count genes before downstream analysis by using at least 20 reads in total (including all samples) by which we removed 6160 genes and kept 22733 genes for downstream analysis. 

## Sample correlation  

There is no logical correlation among samples as shown in figure \@ref(fig:fig-sample-correlation)).  You can see that different organs are clustered together, summer and winter are also together. Library seems not to be the main factor here as well.
 

```{r fig-sample-correlation, echo = F, eval = T}
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/samples_correlation.png')
```

**Heatmap of Euclidean sample distances after rlog transformation**

```{r fig-sample-log, echo = F, eval = T}
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/HeatMap_rlog.png')
```

## PCA  

In figures  \@ref(fig:fig-PCA-sex) and  \@ref(fig:fig-PCA-day) each point corresponds to a sample plotted on PC1 and PC2.  I can't see strong separation by **organ or bat (s/w)**.  **Library** batch effect has been removed.  

```{r fig-PCA-organ, echo = F, eval = T }
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/PCA_bat_lremoved.png')
```

```{r fig-PCA-library, echo = F, eval = T }
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/PCA_library_removed.png')
```
...
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/HeatMap_topvar100.png')
```
...
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/02-DE/HeatMap_topvar100_clus.png')
```


## Annotation 

**Table with the information of the top 100 genes**

Table with all genes can be found at the Github.


```{r tbl, eval = T, echo = F, message=FALSE}
tbl_top <- read.table("/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/01-T/TopVar100.tsv", header=T, row.names=1, check.names=T, sep="\t")
...
Here, you can find the the GO profile of the topVar 100 genes


```{r fig-topVarGenes-GO, echo = F, eval = T }
knitr::include_graphics('/Users/juliana/Documents/GitHub/SMS-6112-22-MB/results/salmon/results/01-T/gProfiler_m.png')
```

**Dinamic Link**

[GO Terms](https://biit.cs.ut.ee/gplink/l/T_bDIpXTTR) 
